...
time_steps = 1
X_gru, y_gru = create_sequences(X_scaled, y_scaled, time_steps)

# Разделение на обучающую и тестовую выборки
split_index = int(0.85 * len(X_gru))
X_train, X_test = X_gru[:split_index], X_gru[split_index:]
y_train, y_test = y_gru[:split_index], y_gru[split_index:]

# Создание модели GRU
model = Sequential()
model.add(GRU(50, activation='relu', input_shape=(time_steps, X_train.shape[2])))
model.add(Dense(1))
...
plt.title(f'Stock Price Prediction using GRU (Full Range, {n_days} Days Ahead)')
plt.legend()
...
plt.xlabel('Date')
plt.ylabel('Stock Prices')
plt.title(f'Stock Price Prediction using GRU (Test Data, {n_days} Days Ahead)')
plt.legend()
